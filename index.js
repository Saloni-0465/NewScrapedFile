...
const placeholder = {
  "Objective": "scheme_objective",
  "Eligibility": "eligibility_criteria",
  "Required Documents / Enclosures with Application": "benefits_provided",
  "Required Enclosures with Application": "benefits_provided",
  "Enclosures with Application": "benefits_provided",
...
async function scrapeSchemes() {
    ...
    for (let i = 0; i < schemes.length; i++) {
        ...
        const details = await page.evaluate((placeholder) => {
            const details = {};
            const table = document.querySelector('.table-responsive table');
            if (!table) {
                return null;
            ...
            rows.forEach(row => {
                ...
                if (categoryElement && detailsElement) {
                    const key = categoryElement.textContent.trim();
                    const value = detailsElement.innerHTML
                        .replace(/<p>/g, '')
                        .replace(/<\/p>/g, '')
                        .replace("a)", '')
                        .replace("b)", '')
